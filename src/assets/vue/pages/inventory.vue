<template>
  <f7-page page-content ptr @ptr:refresh="fetchInventories">

  <f7-navbar title="Inventories" back-link="Back"></f7-navbar>
  <f7-block-title>Total listings: {{inventories.count}}</f7-block-title>

  <f7-block>
     <f7-photo-browser ref="standalone"></f7-photo-browser>
     <f7-card 
        v-for="(acc, key) in inventories.list" :key="key">
        <f7-card-header
              >
        ...
        <f7-block>
           ...
           <f7-list no-hairlines-md inset>
              <f7-list-input label="Type"
                             ...
                             validate
                             >
                             <option v-for="typ in inventories.types" :value="typ">{{typ}}</option>
              </f7-list-input>
              ...
              <f7-list-input label="Change status"
                             ...
                             >
                    <option v-for="st in inventory.status" :value="st">{{st}}</option>
              </f7-list-input>
...
export default {
   ...
   mounted() {
      ...
      // Get all inventory.
      self.postWithPromise( '/inventory/list').then(
         function(json) {
            self.inventories = JSON.parse(json).data;
            self.saveStote('inventories', self.inventories);
      });
   },
   methods: { 
      fetchInventory: function() {
         ...
         const app = self.$f7;
         setTimeout( () => {
            self.fetchAndStore( '/inventory/list', 'inventories');
            self.inventories = JSON.parse(self.$localStorage.get('inventories', '[]'));
            return;
         }, 1000);
         app.ptr.done();
      },
